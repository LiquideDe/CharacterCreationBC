...
public class PointInsert : MonoBehaviour, IDropHandler
{
    ...
    private void OnEnable() => characteristicCard.Reset += ResetBool;    

    private void OnDisable() => characteristicCard.Reset -= ResetBool;
    ...
    public void OnDrop(PointerEventData eventData)
    {
        if (isEmpty)
        {
            if (eventData.pointerDrag != null)
            {
                ...
                characteristicCard.PlusAmount(cardWith.Amount);
